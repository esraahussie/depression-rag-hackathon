...
collection populated by `python main.py`). It works two ways:
  ...
  - If ANTHROPIC_API_KEY is set: calls Claude with the grounding
    prompt and parses its structured JSON response.
...
def generate_answer(query, k=5, use_llm=None):
    """use_llm=None -> auto (use Claude if ANTHROPIC_API_KEY is set,
